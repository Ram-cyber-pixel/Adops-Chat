...
import { faPaperPlane, faPaperclip, faRobot, faUser, faImage, faXmark, faLink, faFlag } from '@fortawesome/free-solid-svg-icons';
...
const allPremadeQuestions = [
  ...
  { text: "Rules for Bookend Pairing LA Uploads", category: "Uploads" },
  { text: "Retail Uploads & QC rotation details", category: "Uploads" },
...
const AvatarContainer = styled.div`
  ...
  height: 36px;
  border-radius: 50%;
  background-color: ${props => props.$isUser ? '#3b82f6' : '#e5e7eb'};
  display: flex;
  align-items: center;
...
const EnhancedAIChat = () => {
  const [messages, setMessages] = useState([
    {
      id: 1,
      text: "Hello! I'm your AI Assistant. I can help with organization processes, answer questions, and provide information with images and links. How can I assist you today?",
      sender: 'bot',
  ...
  const handleMoreTickets = () => {
    if (currentPage * ticketsPerPage < tickets.length) {
      ...
      setTimeout(() => {
        ...
        if (endIndex < tickets.length) {
          response += "\nThere are more tickets available. Type 'more tickets' to see the next page.";
        } else {
